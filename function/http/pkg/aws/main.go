...
// Handle 处理API网关请求
func Handle(ctx context.Context, event server.EventRequest) (*server.EventResponse, error) {
	// 解析经Base64编码的请求体
	decodedBody, err := base64.StdEncoding.DecodeString(event.Body)
	if err != nil {
		return &server.EventResponse{StatusCode: 400}, err
	}
	...
	if err != nil {
		return &server.EventResponse{StatusCode: 500}, err
	}
