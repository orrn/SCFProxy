...
// Handle 处理API网关请求
func Handle(ctx context.Context, event httpserver.EventRequest) (*httpserver.EventResponse, error) {
	// 解析经Base64编码的请求体
	decodedBody, err := base64.StdEncoding.DecodeString(event.Body)
	if err != nil {
		return &httpserver.EventResponse{StatusCode: 400}, err
	}
	...
	if err != nil {
		return &httpserver.EventResponse{StatusCode: 500}, err
	}
	...
	// 构造响应
	return &httpserver.EventResponse{
